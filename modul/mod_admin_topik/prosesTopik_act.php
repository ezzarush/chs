...
if (move_uploaded_file($_FILES['topic_File']['tmp_name'], $target_file)) {
    if($mode==0){
        ...
        $query = "UPDATE as_topics SET topic_ref='$topic_ref', title='$topic_title', title_seo='$topic_title_seo', category_id='$topic_category', sub_category_id='$topic_sub_category',  image='$filename', description='$topic_Description',modified_date='$date' WHERE topic_id='$mode'";
        $db->database_prepare($query)->execute();
    }
